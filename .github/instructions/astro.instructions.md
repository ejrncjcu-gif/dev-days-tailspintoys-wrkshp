...
- Document the `Props` interface with a concise TSDoc comment describing the component's purpose, and document non-obvious props or constraints where needed.
...
- Comment intent and design decisions, not mechanics or markup that is already self-explanatory. Keep comments current when the component changes.
