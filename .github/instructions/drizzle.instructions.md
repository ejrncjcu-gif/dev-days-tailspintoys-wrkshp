...
- Add TSDoc/JSDoc to every exported function in `db/` and `src/lib/`. Document the function's purpose, every parameter (including the injectable `db` argument), and the resolved return value.
- Keep documentation focused on intent and contracts. Do not add comments that merely paraphrase a query or transformation, and update or remove stale comments in the same change as the related code.
