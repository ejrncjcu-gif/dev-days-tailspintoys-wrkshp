...
The storefront homepage includes category and publisher filters so backers can narrow the catalog down to the games that match their preferred genres and studios.
...
Repository-wide coding standards are maintained in the [Copilot instructions](.github/copilot-instructions.md) and the scoped files in [`.github/instructions`](.github/instructions/). In particular, comments should explain intent rather than restate code; exported data-layer functions require TSDoc/JSDoc for their purpose, parameters, and return values; and reusable Astro components document their `Props` interfaces. TypeScript formatting and module-boundary type conventions are enforced by ESLint.
